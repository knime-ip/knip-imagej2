...
/*********************************************************************
 * This class is the only one that is accessed directly by imageJ; it launches a
 * modeless dialog and dies. Note that it implements <code>PlugIn</code> rather
 * than <code>PlugInFilter</code>.
 * 
...

{
	...
	public enum TransformationType {
		GENERIC_TRANSFORMATION(-1), TRANSLATION(2), RIGID_BODY(3), SCALED_ROTATION(
				4), AFFINE(6);
	...
	public void run(final ImagePlus source, final ImagePlus target,
			final TransformationType type, final double[][] sourcePoints,
	 ...
	 * @param target
	 */
	public void transform(final ImagePlus source, final ImagePlus target,
			final TransformationType type, final double[][] sourcePoints,
			final double[][] targetPoints) {
		...
		transformedImage = transformImage(source, source.getWidth(),
	 ...
	 * <code>outputWidth</code> and <code>outputHeight</code> parameters of the
	 * call to the <code>run</code> method of <code>TurboReg_</code>.
	 * 
	 * @see TurboReg_#run
	 ********************************************************************/
	 ...
	 */
	/*------------------------------------------------------------------*/
	private ImagePlus alignImages(final ImagePlus source,
			final ImagePlus target, final TransformationType transformation) {
		if ((source.getType() != ImagePlus.GRAY16)
				&& (source.getType() != ImagePlus.GRAY32)
				&& ((source.getType() != ImagePlus.GRAY8)
						|| source.getStack().isRGB() || source.getStack()
						.isHSB())) {
			IJ.error(source.getTitle()
					+ " should be grayscale (8, 16, or 32 bit)");
			return (null);
		}
		if ((target.getType() != ImagePlus.GRAY16)
				&& (target.getType() != ImagePlus.GRAY32)
				&& ((target.getType() != ImagePlus.GRAY8)
						|| target.getStack().isRGB() || target.getStack()
						.isHSB())) {
			IJ.error(target.getTitle()
					+ " should be grayscale (8, 16, or 32 bit)");
			return (null);
		}
		...
		source.setSlice(1);
		target.setSlice(1);
		final ImagePlus sourceImp = new ImagePlus("source", source
				.getProcessor().crop());
		final ImagePlus targetImp = new ImagePlus("target", target
				.getProcessor().crop());
		final turboRegImage sourceImg = new turboRegImage(sourceImp,
				transformation, false);
		final turboRegImage targetImg = new turboRegImage(targetImp,
				transformation, true);
		final int pyramidDepth = getPyramidDepth(sourceImp.getWidth(),
				sourceImp.getHeight(), targetImp.getWidth(),
				targetImp.getHeight());
		...
		final ImagePlus sourceMskImp = new ImagePlus("source mask", source
				.getProcessor().crop());
		final ImagePlus targetMskImp = new ImagePlus("target mask", target
				.getProcessor().crop());
		...
		final turboRegPointHandler sourcePh = (null == sourceImp.getWindow()) ? (new turboRegPointHandler(
				transformation, sourceImp)) : (new turboRegPointHandler(
				sourceImp, transformation));
		final turboRegPointHandler targetPh = (null == sourceImp.getWindow()) ? (new turboRegPointHandler(
				transformation, targetImp)) : (new turboRegPointHandler(
		...
		final turboRegFinalAction finalAction = new turboRegFinalAction(
				sourceImg, sourceMsk, sourcePh, targetImg, targetMsk, targetPh,
				transformation);
	...
	/*------------------------------------------------------------------*/
	private ImagePlus transformImage(final ImagePlus source, final int width,
		...
		final turboRegImage sourceImg = new turboRegImage(source,
				TransformationType.GENERIC_TRANSFORMATION, false);
		sourceImg.getThread().start();
		if (2 <= source.getStackSize()) {
			source.setSlice(2);
		}
		...
		final turboRegTransform regTransform = new turboRegTransform(sourceImg,
				sourceMsk, sourcePh, null, null, targetPh, transformation,
				false, false);
		final ImagePlus transformedImage = regTransform.doFinalTransform(width,
				height);
		return (transformedImage);
	} /* end transformImage */
...
/*********************************************************************
 * The purpose of this class is to allow access to the progress bar, since it is
 * denied to the <code>turboRegDialog</code> class. It proceeds by wrapping
 * <code>turboRegDialog</code> inside a thread that is under the main event loop
 * control.
 ********************************************************************/
...
{ /* class turboRegFinalAction */
	 ...
	 */
	/*********************************************************************
	 * Automatic registration: the initial source landmarks are refined to
	 * minimize the mean-square error.
	 ********************************************************************/
	public static final int AUTOMATIC = 1;

	/*********************************************************************
	 * Manual registration: the initial source landmarks are used <i>as is</i>
	 * to produce the output image.
	 ********************************************************************/
	public static final int MANUAL = 2;

	/*********************************************************************
	 * Batch registration: each slice of the source image is registered to the
	 * target image.
	 ********************************************************************/
	 ...
	 */
	/*********************************************************************
	 * Start under the control of the main event loop, pause as long as the
	 * dialog event loop is active, and resume processing when
	...
	public void run() {
		...
		case MANUAL: {
			...
			outputImp = tt.doFinalTransform(targetImg.getWidth(),
					targetImg.getHeight());
			break;
		}
		case BATCH: {
			...
			for (int i = 0; (i < sourceImp.getStackSize()); i++) {
				outputStack.addSlice(
						"",
						new FloatProcessor(targetImg.getWidth(), targetImg
								.getHeight()));
			}
			...
			tt = new turboRegTransform(sourceImg, null, sourcePh, targetImg,
					targetMsk, targetPh, transformation, accelerated, false);
			...
			tt.doRegistration();
			tt.doBatchFinalTransform((float[]) outputStack.getProcessor(1)
					.getPixels());
			...
			for (int i = 2; (i <= sourceImp.getStackSize()); i++) {
				...
				try {
					...
					IJ.log("Unexpected interruption exception "
							+ e.getMessage());
				}
				...
				tt.doRegistration();
				tt.doBatchFinalTransform((float[]) outputStack.getProcessor(i)
						.getPixels());
	 ...
	 */
	/*********************************************************************
	 * Start a thread under the control of the main event loop. This thread has
	 * access to the progress bar, while methods called directly from within
	 * <code>turboRegDialog</code> do not because they are under the control of
	 * its own event loop.
	 * 
	 * @param dialog
	 *            Gives access to some utility methods within
	 ...
	 ********************************************************************/
	public turboRegFinalAction(final turboRegImage sourceImg,
			final turboRegMask sourceMsk, final turboRegPointHandler sourcePh,
			final turboRegImage targetImg, final turboRegMask targetMsk,
	 ...
	 * @param saveOnExit
	 ********************************************************************/
	public void setup(final turboRegImage sourceImg,
			final turboRegMask sourceMsk, final turboRegPointHandler sourcePh,
			final turboRegImage targetImg, final turboRegMask targetMsk,
			final turboRegPointHandler targetPh,
			final TransformationType transformation, final boolean accelerated,
			final boolean saveOnExit, final int operation) {
		this.sourceImg = sourceImg;
		this.sourceMsk = sourceMsk;
		this.sourcePh = sourcePh;
		this.targetImg = targetImg;
	 ...
	 * @param saveOnExit
	 ********************************************************************/
	public void setup(final ImagePlus sourceImp, final turboRegImage sourceImg,
			final turboRegMask sourceMsk, final turboRegPointHandler sourcePh,
			final int sourceColorPlane, final turboRegImage targetImg,
	 ...
	 ********************************************************************/
	public void setup(final ImagePlus sourceImp, final turboRegImage sourceImg,
			final turboRegPointHandler sourcePh, final ImagePlus targetImp,
			final turboRegImage targetImg, final turboRegMask targetMsk,
			final turboRegPointHandler targetPh,
			final TransformationType transformation, final boolean accelerated,
			final boolean saveOnExit, final int pyramidDepth) {
		this.sourceImp = sourceImp;
		this.sourceImg = sourceImg;
		this.sourcePh = sourcePh;
...
/*********************************************************************
 * This class is responsible for the image preprocessing that takes place
 * concurrently with user-interface events. It contains methods to compute
 * B-spline coefficients and their pyramids, image pyramids, gradients, and
 * gradient pyramids.
 ********************************************************************/
...
{ /* class turboRegImage */
	 ...
	 */
	/*********************************************************************
	 * Converts the pixel array of the incoming <code>ImagePlus</code> object
	 * into a local <code>float</code> array.
	 * 
	 * @param imp
	 ...
	 ********************************************************************/
	public turboRegImage(final ImagePlus imp,
			final TransformationType transformation, final boolean isTarget) {
		t = new Thread(this);
		t.setDaemon(true);
		this.transformation = transformation;
	 ...
	 * of the stack depends on whether the <code>turboRegImage</code> object
	 * corresponds to the target or the source image, and on the transformation
	 * (ML* = { <code>TRANSLATION</code>,<code>RIGID_BODY</code>,
	 * <code>SCALED_AFFINE</code>, <code>AFFINE</code> vs. ML = {
 ...
 <tr><th><code>isTarget</code></th> <th>ML*</th> <th>ML</th></tr>
	...
	/*********************************************************************
	 * Return the depth of the image pyramid. A depth <code>1</code> means that
	 * one coarse resolution level is present in the stack. The full-size level
	 ...
	 */
	/*------------------------------------------------------------------*/
	private void antiSymmetricFirMirrorOffBounds1D(final double[] h,
	...
	/*------------------------------------------------------------------*/
	private void basicToCardinal2D(final float[] basic, final float[] cardinal,
	...
	private void buildCoefficientPyramid() {
		...
		for (int depth = 1; ((depth < pyramidDepth) && (!t.isInterrupted())); depth++) {
			...
			final float[] halfDual = getHalfDual2D(fullDual, fullWidth,
					fullHeight);
			final float[] halfCoefficient = getBasicFromCardinal2D(halfDual,
	...
	private void buildImageAndGradientPyramid() {
		...
		for (int depth = 1; ((depth < pyramidDepth) && (!t.isInterrupted())); depth++) {
			...
			final float[] halfDual = getHalfDual2D(fullDual, fullWidth,
					fullHeight);
			final float[] halfImage = getBasicFromCardinal2D(halfDual,
					halfWidth, halfHeight, 7);
	...
	/*------------------------------------------------------------------*/
	private void cardinalToDual2D(final float[] cardinal, final float[] dual,
			final int width, final int height, final int degree) {
		basicToCardinal2D(
				getBasicFromCardinal2D(cardinal, width, height, degree), dual,
	...
	/*------------------------------------------------------------------*/
	private void coefficientToXYGradient2D(final float[] basic,
			final float[] xGradient, final float[] yGradient, final int width,
			final int height) {
		final double[] hLine = new double[width];
		final double[] hData = new double[width];
		final double[] vLine = new double[height];
	...
	/*------------------------------------------------------------------*/
	private void extractColumn(final float[] array, final int width, int x,
			final double[] column) {
	...
	/*------------------------------------------------------------------*/
	private float[] getBasicFromCardinal2D(final float[] cardinal,
			final int width, final int height, final int degree) {
		final float[] basic = new float[width * height];
		final double[] hLine = new double[width];
		final double[] vLine = new double[height];
	...
	/*------------------------------------------------------------------*/
	private float[] getHalfDual2D(final float[] fullDual, final int fullWidth,
	...
	private double getInitialAntiCausalCoefficientMirrorOffBounds(
			final double[] c, final double z, final double tolerance) {
		return ((z * c[c.length - 1]) / (z - 1.0));
	} /* end getInitialAntiCausalCoefficientMirrorOffBounds */

	/*------------------------------------------------------------------*/
	private double getInitialCausalCoefficientMirrorOffBounds(final double[] c,
			final double z, final double tolerance) {
		double z1 = z;
	...
	/*------------------------------------------------------------------*/
	private void putColumn(final float[] array, final int width, int x,
			final double[] column) {
		for (int i = 0; (i < column.length); i++) {
	...
	/*------------------------------------------------------------------*/
	private void samplesToInterpolationCoefficient1D(final double[] c,
			final int degree, final double tolerance) {
		...
		for (int k = 0; (k < z.length); k++) {
			c[0] = getInitialCausalCoefficientMirrorOffBounds(c, z[k],
					tolerance);
			for (int n = 1; (n < c.length); n++) {
				c[n] = c[n] + (z[k] * c[n - 1]);
			}
			c[c.length - 1] = getInitialAntiCausalCoefficientMirrorOffBounds(c,
					z[k], tolerance);
	...
	/*------------------------------------------------------------------*/
	private void symmetricFirMirrorOffBounds1D(final double[] h,
			final double[] c, final double[] s) {
		switch (h.length) {
		case 2: {
			if (2 <= c.length) {
...
/*********************************************************************
 * This class is responsible for the mask preprocessing that takes place
 * concurrently with user-interface events. It contains methods to compute the
 * mask pyramids.
 ********************************************************************/
...
{ /* class turboRegMask */
	...
	/*********************************************************************
	 * Converts the pixel array of the incoming <code>ImagePlus</code> object
	 * into a local <code>boolean</code> array.
	 * 
	 * @param imp
	 *            <code>ImagePlus</code> object to preprocess.
	 ********************************************************************/
	...
	/*********************************************************************
	 * Return the pyramid as a <code>Stack</code> object. A single pyramid level
	 * consists of
	 * <p>
	 * <table border="1">
	 * <tr>
	...
	private void buildPyramid() {
		...
		for (int depth = 1; ((depth < pyramidDepth) && (!t.isInterrupted())); depth++) {
			...
			halfWidth /= 2;
			halfHeight /= 2;
			final float[] halfMask = getHalfMask2D(fullMask, fullWidth,
					fullHeight);
	...
	/*------------------------------------------------------------------*/
	private float[] getHalfMask2D(final float[] fullMask, final int fullWidth,
			final int fullHeight) {
		final int halfWidth = fullWidth / 2;
		final int halfHeight = fullHeight / 2;
...
/*********************************************************************
 * This class implements the various listeners that are in charge of user
 * interactions when dealing with landmarks. It overrides the listeners of
 * ImageJ, if any. Those are restored upon restitution of this
 * <code>ImageCanvas</code> object to ImageJ.
 ********************************************************************/
class turboRegPointAction extends ImageCanvas implements AdjustmentListener,
...
{ /* class turboRegPointAction */
	 ...
	 *            <ul>
	 *            <li><code>KeyEvent.VK_COMMA</code>: display the previous
	 *            slice, if any;</li>
	 *            <li><code>KeyEvent.VK_DOWN</code>: move down the current
	 *            landmark;</li>
	 *            <li><code>KeyEvent.VK_LEFT</code>: move the current landmark
	 *            to the left;</li>
	 *            <li><code>KeyEvent.VK_PERIOD</code>: display the next slice,
	 *            if any;</li>
	 *            <li><code>KeyEvent.VK_SPACE</code>: select the current
	 *            landmark;</li>
	 *            <li><code>KeyEvent.VK_RIGHT</code>: move the current landmark
	 *            to the right;</li>
	 *            <li><code>KeyEvent.VK_UP</code>: move up the current landmark.
	 *            </li>
	 *            </ul>
	 ********************************************************************/
	@Override
	public void keyPressed(final KeyEvent e) {
		...
		case KeyEvent.VK_DOWN: {
			mainPh.movePoint(
					mainImp.getWindow().getCanvas().screenX(x),
					mainImp.getWindow()
							.getCanvas()
							.screenY(
									y
											+ (int) Math.ceil(1.0 / mainImp
													.getWindow().getCanvas()
													.getMagnification())));
	...
	/*********************************************************************
	 * Listen to <code>mousePressed</code> events. Update the current point or
	 * call the ImageJ's zoom methods.
	 * 
	 * @param e
	 *            Event.
	 ********************************************************************/
	 ...
	 */
	/*********************************************************************
	 * Listen to <code>mouseDragged</code> events. Move the position of the
	 * current point. Update the window's ROI. Update ImageJ's window.
	 * 
	 * @param e
	 *            Event.
	...
	/*********************************************************************
	 * Listen to <code>mouseMoved</code> events. Update the ImageJ status by
	 * displaying the value of the pixel under the cursor hot spot.
	 * 
	 * @param e
	 *            Event.
	 ********************************************************************/
	...
	public void mouseMoved(final MouseEvent e) {
		...
		IJ.showStatus(mainImp.getLocationAsString(x, y)
	 ...
	 ********************************************************************/
	public turboRegPointAction(final ImagePlus imp,
			final turboRegPointHandler ph, final turboRegPointToolbar tb) {
		super(imp);
		this.mainImp = imp;
		this.mainPh = ph;
		this.tb = tb;
	 ...
	 *            <code>turboRegPointHandler</code> object.
	 ********************************************************************/
	public void setSecondaryPointHandler(final ImagePlus secondaryImp,
...
{ /* class turboRegPointHandler */
	 ...
	 ********************************************************************/
	public turboRegPointHandler(final double[][] precisionPoint,
			final TransformationType transformation) {
	...
	/*********************************************************************
	 * Keep a local copy of the <code>ImagePlus</code> object. Set the landmarks
	 * to their initial position for the given transformation.
	 * 
	 * @param imp
	 *            <code>ImagePlus</code> object.
	 * @param transformation
	 ...
	 ********************************************************************/
	public turboRegPointHandler(final ImagePlus imp,
			final TransformationType transformation) {
		super(0, 0, imp);
		this.imp = imp;
		this.transformation = transformation;
	 ...
	 * @see turboRegDialog#restoreAll()
	 ********************************************************************/
	public turboRegPointHandler(final TransformationType transformation,
			final ImagePlus imp) {
		super(new Polygon(), POLYGON);
		this.imp = imp;
		this.transformation = transformation;
	 ...
	 * <code>double[transformation / 2][2]</code>, except for a rigid-body
	 * transformation for which the array has size <code>double[3][2]</code> .
	 ********************************************************************/
	public double[][] getPoints() {
		if (interactive) {
			if (transformation == TransformationType.RIGID_BODY) {
				final double[][] points = new double[transformation
						.getNumParam()][2];
				for (int k = 0; (k < transformation.getNumParam()); k++) {
	...
	public void movePoint(int x, int y) {
		...
		if ((transformation == TransformationType.RIGID_BODY)
			...
			final double radius = 0.5 * Math
					.sqrt(((ic.screenX(p.x) - ic.screenX(q.x)) * (ic
							.screenX(p.x) - ic.screenX(q.x)))
							+ ((ic.screenY(p.y) - ic.screenY(q.y)) * (ic
									.screenY(p.y) - ic.screenY(q.y))));
	...
	public void setTransformation(final TransformationType transformation) {
		...
		switch (transformation) {
		case TRANSLATION: {
			point[0] = new Point(Math.round((float) (Math.floor(0.5 * width))),
		...
		case RIGID_BODY: {
			...
			point[2] = new Point(Math.round((float) (Math.floor(0.5 * width))),
					height
		...
		case SCALED_ROTATION: {
			point[0] = new Point(Math.round((float) (Math.floor(0.25
					* GOLDEN_RATIO * width))), Math.round((float) (Math
					.floor(0.5 * height))));
			point[1] = new Point(width
					- Math.round((float) (Math
							.ceil(0.25 * GOLDEN_RATIO * width))),
					Math.round((float) (Math.floor(0.5 * height))));
			break;
		}
		case AFFINE: {
			point[0] = new Point(Math.round((float) (Math.floor(0.5 * width))),
					Math.round((float) (Math
							.floor(0.25 * GOLDEN_RATIO * height))));
			point[1] = new Point(Math.round((float) (Math.floor(0.25
					* GOLDEN_RATIO * width))), height
					- Math.round((float) (Math.ceil(0.25 * GOLDEN_RATIO
							* height))));
			point[2] = new Point(width
					- Math.round((float) (Math
							.ceil(0.25 * GOLDEN_RATIO * width))), height
					- Math.round((float) (Math.ceil(0.25 * GOLDEN_RATIO
							* height))));
			break;
		}
...
/*********************************************************************
 * This class implements the user interactions when dealing with the toolbar in
 * the ImageJ's window.
 ********************************************************************/
...
{ /* class turboRegPointToolbar */
	 ...
	 */
	/*********************************************************************
	 * Override the ImageJ toolbar by this <code>turboRegToolbar</code> object.
	 * Store a local copy of the ImageJ's toolbar for later restore.
	 * 
...
/*********************************************************************
 * This class implements the interactions when dealing with ImageJ's progress
 * bar.
 ********************************************************************/
...
/*********************************************************************
 * This class implements the algorithmic methods of the plugin. It refines the
 * landmarks and computes the final images.
 ********************************************************************/
...
{ /* class turboRegTransform */
	 ...
	 */
	/*********************************************************************
	 * Maximal number of registration iterations per level, when speed is
	 * requested at the expense of accuracy. This number must be corrected so
	 * that there are more iterations at the coarse levels of the pyramid than
	 * at the fine levels.
	 * 
	...
	/*********************************************************************
	 * Maximal number of registration iterations per level, when accuracy is
	 * requested at the expense of speed. This number must be corrected so that
	 * there are more iterations at the coarse levels of the pyramid than at the
	 * fine levels.
	 * 
	 * @see turboRegTransform#ITERATION_PROGRESSION
	 ********************************************************************/
	...
	/*********************************************************************
	 * Minimal update distance of the landmarks, in pixel units, when accuracy
	 * is requested at the expense of speed. This distance does not depend on
	 * the pyramid level.
	 ********************************************************************/
	private static final double PIXEL_HIGH_PRECISION = 0.001;

	/*********************************************************************
	 * Minimal update distance of the landmarks, in pixel units, when speed is
	 * requested at the expense of accuracy. This distance does not depend on
	 * the pyramid level.
	 ********************************************************************/
	private static final double PIXEL_LOW_PRECISION = 0.1;

	/*********************************************************************
	 * Multiplicative factor that determines how many more iterations are
	 * allowed for a pyramid level one unit coarser.
	 ********************************************************************/
	 ...
	 ********************************************************************/
	public turboRegTransform(final turboRegImage sourceImg,
			final turboRegMask sourceMsk, final turboRegPointHandler sourcePh,
			final turboRegImage targetImg, final turboRegMask targetMsk,
			final turboRegPointHandler targetPh,
			final TransformationType transformation, final boolean accelerated,
	...
	public void doBatchFinalTransform(final float[] pixels) {
		...
		final double[][] matrix = getTransformationMatrix(targetPoint,
				sourcePoint);
		switch (transformation) {
		case TRANSLATION: {
	 ...
	 * Compute the final image.
	 ********************************************************************/
	public float[] doFinalTransform(final turboRegImage sourceImg,
	...
	public void doRegistration() {
		...
		turboRegProgressBar
				.addWorkload(((pyramidDepth * maxIterations * iterationPower) / ITERATION_PROGRESSION)
		...
		while (!targetImgPyramid.isEmpty()) {
			...
			case SCALED_ROTATION: {
				targetJacobian = ((targetPoint[0][0] - targetPoint[1][0]) * (targetPoint[0][0] - targetPoint[1][0]))
		...
		iterationPower = (int) Math.pow(ITERATION_PROGRESSION, pyramidDepth);
		turboRegProgressBar
				.workloadDone(((pyramidDepth * maxIterations * iterationPower) / ITERATION_PROGRESSION)
						- ((iterationPower - 1) / (ITERATION_PROGRESSION - 1)));
	} /* end doRegistration */
	...
	/*------------------------------------------------------------------*/
	private double getAffineMeanSquares(final double[][] sourcePoint,
			final double[][] matrix) {
	...
	/*------------------------------------------------------------------*/
	private double getAffineMeanSquares(final double[][] sourcePoint,
			final double[][] matrix, final double[] gradient) {
		final double u1 = sourcePoint[0][0];
		final double u2 = sourcePoint[1][0];
		final double u3 = sourcePoint[2][0];
		final double v1 = sourcePoint[0][1];
	...
	/*------------------------------------------------------------------*/
	private double getAffineMeanSquares(final double[][] sourcePoint,
			final double[][] matrix, final double[][] hessian,
				...
				for (int u = 0; (u < outNx); u++, k++) {
					...
					if ((0 <= xMsk) && (xMsk < inNx) && (0 <= yMsk)
						...
						if (inMsk[(yMsk * inNx) + xMsk] != 0.0F) {
							...
							difference = outImg[k] - interpolate();
							meanSquares += difference * difference;
							gradient[0] += difference
									* ((yGradient[k] * (double) u) - (xGradient[k] * (double) v));
	...
	/*------------------------------------------------------------------*/
	private double getScaledRotationMeanSquares(final double[][] sourcePoint,
			final double[][] matrix) {
		final double u1 = sourcePoint[0][0];
		final double u2 = sourcePoint[1][0];
		final double v1 = sourcePoint[0][1];
	...
	/*------------------------------------------------------------------*/
	private double[][] getTransformationMatrix(final double[][] fromCoord,
		...
		case RIGID_BODY: {
			...
			matrix[0][0] = (toCoord[0][0] - (c * fromCoord[0][0]))
					+ (s * fromCoord[0][1]);
			matrix[0][1] = c;
			matrix[0][2] = -s;
			matrix[1][0] = toCoord[0][1] - (s * fromCoord[0][0])
					- (c * fromCoord[0][1]);
			matrix[1][1] = s;
			matrix[1][2] = c;
			break;
	...
	private void inverseMarquardtLevenbergOptimization(int workload) {
		...
		final double[][] hessian = new double[transformation.getNumParam()][transformation
				.getNumParam()];
		...
		switch (transformation) {
		case TRANSLATION: {
			bestMeanSquares = getTranslationMeanSquares(matrix, hessian,
					gradient);
			break;
		}
		case SCALED_ROTATION: {
			bestMeanSquares = getScaledRotationMeanSquares(sourcePoint, matrix,
					hessian, gradient);
			break;
		}
		case AFFINE: {
			bestMeanSquares = getAffineMeanSquares(sourcePoint, matrix,
					hessian, gradient);
			break;
		}
		}
		...
		do {
			...
			case TRANSLATION: {
				if (accelerated) {
					...
					meanSquares = getTranslationMeanSquares(matrix, hessian,
							gradient);
				}
				break;
			}
	...
	private void inverseMarquardtLevenbergRigidBodyOptimization(int workload) {
		...
		int iteration = 0;
		for (int k = 0; (k < transformation.getNumParam()); k++) {
			sourcePoint[k][0] = matrix[0][0]
					+ (targetPoint[k][0] * matrix[0][1])
					+ (targetPoint[k][1] * matrix[0][2]);
			sourcePoint[k][1] = matrix[1][0]
					+ (targetPoint[k][0] * matrix[1][1])
		...
		do {
			...
			displacement = Math.sqrt((update[1] * update[1])
					+ (update[2] * update[2]))
					+ (0.25 * Math.sqrt((double) (inNx * inNx)
							+ (double) (inNy * inNy)) * Math.abs(update[0]));
			if (accelerated) {
				meanSquares = getRigidBodyMeanSquares(attempt, gradient);
			} else {
				meanSquares = getRigidBodyMeanSquares(attempt, hessian,
						gradient);
			}
			iteration++;
			if (meanSquares < bestMeanSquares) {
	...
	/*------------------------------------------------------------------*/
	private double[] matrixMultiply(final double[][] matrix,
			final double[] vector) {
