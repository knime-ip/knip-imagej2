...
public class ImgOutputAdapter implements IJOutputAdapter<Img> {
    ...
    @Override
    public IJOutputAdapterInstance<Img> createAdapterInstance(final ExecutionContext exec) {

        final ImgPlusCellFactory factory = new ImgPlusCellFactory(exec);

        return new IJOutputAdapterInstance<Img>() {

            @Override
            public DataCell[] getDataCells(final Img ijObject) {
                ImgPlusCell output = null;
                try {
                    output = factory.createCell(new ImgPlus(ijObject));
