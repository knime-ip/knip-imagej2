...
/**
 * Converts ImageJ output parameters of type primitive double to KNIME {@link DoubleValue}/{@link DoubleCell}.
 *
...
public class PDoubleOutputAdapter implements IJOutputAdapter {

    private final NodeLogger LOGGER = NodeLogger.getLogger(PDoubleOutputAdapter.class);
    ...
    public IJOutputAdapterInstance createAdapterInstance(final ExecutionContext exec) {
        return new IJOutputAdapterInstance() {
            ...
            public DataCell[] getDataCells(final Object ijObject) {
                if (ijObject instanceof Double) {
                    return new DataCell[]{new DoubleCell(((Double)ijObject))};
                } else {
                    LOGGER.error("Primitive adapter for type double can not handle objects of type "
                            + ijObject.getClass());
                    return new DataCell[]{new DoubleCell(-1.0d)};
                }
            }
        };
    }
