...
/**
 * Converts ImageJ output parameters of type primitive char to KNIME {@link StringValue}/{@link StringCell}.
 *
...
public class PCharacterOutputAdapter implements IJOutputAdapter {

    private final NodeLogger LOGGER = NodeLogger.getLogger(PCharacterOutputAdapter.class);
    ...
    public IJOutputAdapterInstance createAdapterInstance(final ExecutionContext exec) {
        return new IJOutputAdapterInstance() {
            ...
            public DataCell[] getDataCells(final Object ijObject) {
                if (ijObject instanceof Character) {
                    return new DataCell[]{new StringCell(((Character)ijObject).toString())};
                } else {
                    LOGGER.error("Primitive adapter for type char can not handle objects of type "
                            + ijObject.getClass());
                    return new DataCell[]{new StringCell("")};
                }
            }
        };
    }
